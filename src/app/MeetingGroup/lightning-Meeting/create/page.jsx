...
import TextField from "@mui/material/TextField"; // 기본 TextField import
...
export default function LightningMeetingCreatePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "번개 모임 생성에 실패했습니다. 다시 시도해주세요.");
      }

      setSnackbarMessage("새로운 번개 모임이 성공적으로 작성되었습니다.");
      ...
      console.error("번개 모임 생성 오류:", error.message);
  ...
  // 버튼 비활성화 여부 (isFormValid 제거)
  // 필요에 따라 폼의 모든 필드가 유효한지 확인
      ...
      >
        ...
        >
          <StyledSendButton
            ...
            disabled={isLoading}
          >
            {isLoading ? <CircularProgress size={24} color="inherit" /> : "작성"}
          </StyledSendButton>
          <StyledCancelButton
            variant="contained"
