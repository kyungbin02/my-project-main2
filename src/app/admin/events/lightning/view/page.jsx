...
export default function Page() {
  ...
  const handleDetailClick = (post_idx) => {
    router.push(`/admin/events/lightning/detail/${post_idx}`); // 디테일 페이지로 이동
  };
    ...
    <>
      <Box sx={{ display: "flex" }}>
        ...
        >
          ...
          >
            <h3 style={{ color: "black" }}>번개 모임 정보</h3>
            <TableContainer
              component={Paper}
                ...
                <Table className="camping-table">
                  ...
                  <TableBody>
                    {pagedMembers.map((item, index) => (
                      <TableRow
                        key={index}
                        onClick={() => handleDetailClick(item.post_idx)}
                      >
                        <TableCell>{item.post_idx}</TableCell>
                        <TableCell>{item.title}</TableCell>
                        <TableCell>{item.writer_idx}</TableCell>
                        <TableCell>{item.meeting_date}</TableCell>
                        <TableCell>{item.meeting_location}</TableCell>
